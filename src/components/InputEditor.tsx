...
interface InputEditorV2Props extends Omit<UIInputFieldProps, "form" | "type"> {
  maxLength?: number;
  context?: string;
...
const InputEditorV2 = (props: InputEditorV2Props) => {
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      ...
      form.setValue(name, html, { shouldValidate: false, shouldDirty: true });
  ...
  const generateContentMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.success && result.content && editor) {
        ...
        // Manually update the form value to ensure it's synchronized
        form.setValue(name, finalContent, {
          shouldValidate: true,
          shouldDirty: true,
          shouldTouch: true,
        });
